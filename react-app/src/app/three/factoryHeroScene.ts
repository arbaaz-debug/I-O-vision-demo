...
/**
 * Rotating 3D factory showpiece for the landing hero — framework-agnostic.
 * (Ported verbatim from the Angular FactoryHeroComponent.)
 */
interface Puff {
  mesh: THREE.Mesh;
  phase: number;
...
interface Plume { group: THREE.Group; puffs: Puff[]; }

export class FactoryHeroScene {
  ...
  private M!: {
    concrete: THREE.MeshStandardMaterial; concreteDark: THREE.MeshStandardMaterial;
    painted: THREE.MeshStandardMaterial; paintedLight: THREE.MeshStandardMaterial;
    darkSteel: THREE.MeshStandardMaterial; stainless: THREE.MeshStandardMaterial;
    copper: THREE.MeshStandardMaterial; glass: THREE.MeshStandardMaterial;
    red: THREE.MeshStandardMaterial; accent: THREE.MeshStandardMaterial;
    grass: THREE.MeshStandardMaterial; water: THREE.MeshStandardMaterial;
    ground: THREE.MeshStandardMaterial; rim: THREE.MeshStandardMaterial;
  };
  ...
  private buildScene(): void {
    ...
    const wall = new THREE.Mesh(this.track(new THREE.PlaneGeometry(70, 38)), this.track(new THREE.ShadowMaterial({ opacity: 0.3 })));
    wall.position.set(0, 9, -7);
    wall.receiveShadow = true;
    this.scene.add(wall);
    const ground = new THREE.Mesh(this.track(new THREE.PlaneGeometry(80, 80)), this.track(new THREE.ShadowMaterial({ opacity: 0.22 })));
  ...
  private buildBoilerHouse(): void {
    ...
    for (let i = -1; i <= 1; i++) this.cyl(0.26, 0.26, 0.7, this.M.stainless, i * 3.2, 3.95, 1.1, g);
  }
  ...
  private buildChimneys(): void {
    const make = (x: number) => {
      ...
      const cap = new THREE.Mesh(this.track(new THREE.TorusGeometry(0.52, 0.07, 10, 28)), this.M.darkSteel);
      cap.rotation.x = Math.PI / 2;
      cap.position.y = 11.05;
      cap.castShadow = true;
      g.add(cap);
      this.addPlume(g, 0, 11.2, 0, { count: 6, rate: 0.16, rise: 5.5, spread: 0.34, from: 0.5, to: 2.2, opacity: 0.5, drift: 0.6, warm: true });
    };
    make(2.7); make(4.5);
  }
  private buildCoolingTowers(): void {
    const make = (x: number, z: number) => {
      ...
      this.factory.add(g);
      this.cyl(1.55, 1.75, 0.3, this.M.concreteDark, 0, 0.15, 0, g, 32);
      this.cyl(1.28, 1.7, 4.2, this.M.concrete, 0, 2.3, 0, g, 32);
      this.cyl(1.4, 1.28, 0.35, this.M.concreteDark, 0, 4.4, 0, g, 32);
      this.addPlume(g, 0, 4.5, 0, { count: 7, rate: 0.13, rise: 4.6, spread: 0.9, from: 1.1, to: 3.0, opacity: 0.55, drift: 0.5, warm: false });
    };
    make(-6.6, 2.2); make(-8.2, -0.6);
  }
  ...
  private buildSwitchyard(): void {
    ...
    for (let i = 0; i < 3; i++) {
      const x = i * 1.4;
      this.box(0.14, 3.2, 0.14, this.M.darkSteel, x, 1.6, 0, g);
      this.box(1.4, 0.1, 0.1, this.M.darkSteel, x, 2.5, 0, g);
      this.box(1.1, 0.1, 0.1, this.M.darkSteel, x, 2.9, 0, g);
    }
    const xf = (x: number) => {
  ...
  private buildCameraRig(): void { this.buildSuspendedCamera(-1); this.buildSuspendedCamera(1); }
  private buildSuspendedCamera(side: number): void {
    const x = 10 * side; const z = 3; const topY = 8.8; const camY = 7.6;
    ...
    pivot.rotation.y = Math.atan2(-x, -z);
    ...
    const ledMat = this.track(new THREE.MeshStandardMaterial({ color: 0x2a0806, emissive: 0xff2a20, emissiveIntensity: 0, roughness: 0.4 }));
    ...
    rig.traverse((o) => { if ((o as THREE.Mesh).isMesh) (o as THREE.Mesh).castShadow = false; });
  }

  private addPlume(parent: THREE.Object3D, x: number, y: number, z: number, o: { count: number; rate: number; rise: number; spread: number; from: number; to: number; opacity: number; drift: number; warm: boolean }): void {
    ...
    for (let i = 0; i < o.count; i++) {
      const mat = this.track(new THREE.MeshStandardMaterial({ color, transparent: true, opacity: 0, depthWrite: false, roughness: 1, metalness: 0 }));
      const ox = (Math.random() - 0.5) * o.spread;
      const mesh = new THREE.Mesh(geo, mat);
      mesh.position.set(ox, 0, (Math.random() - 0.5) * o.spread);
      group.add(mesh);
      puffs.push({ mesh, phase: i / o.count, speed: o.rate * (0.8 + Math.random() * 0.4), rise: o.rise, drift: o.drift, ox, seed: Math.random() * 6.2831, from: o.from, to: o.to, baseOpacity: o.opacity });
    }
  ...
  private tickPlumes(dt: number): void {
    for (const plume of this.plumes) {
      for (const p of plume.puffs) {
        ...
        (p.mesh.material as THREE.MeshStandardMaterial).opacity = Math.sin(Math.PI * k) * p.baseOpacity;
      }
    }
  }
  ...
  private cyl(rt: number, rb: number, h: number, mat: THREE.Material, x: number, y: number, z: number, parent: THREE.Object3D, seg = 20): THREE.Mesh {
